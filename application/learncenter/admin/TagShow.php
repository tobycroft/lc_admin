...
class TagShow extends Admin
{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = TagShowModel::where($map)
            ->order($order)
            ->paginate();
...
//            ->setPageTips("总数量：" . $num2, 'danger')
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($UserInfo = TagShowModel::create($data)) {
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (TagShowModel::update($data)) {
                $UserInfo = TagShowModel::get($data['id']);
                // 记录行为
                $this->success('编辑成功');
            } else {
                $this->error('编辑失败');
    ...
    public function access($module = '', $uid = 0, $tab = '')
    {
        ...
        if (session('UserInfo_auth.role') != 1) {
            ...
            $UserInfo_list = TagShowModel::where('role', 'in', $role_list)
                ->column('id');
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === TagShowModel::where('id', 'in', $ids)
                        ->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
