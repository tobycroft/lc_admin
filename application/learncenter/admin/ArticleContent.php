...
class ArticleContent extends Admin
{
    ...
    public function index()
    {
        ...
        $num1 = ArticleContentModel::where("date", ">", $todaytime)
            ->count();
        ...
        $btn_access = [
            'title' => '用户地址',
            'icon' => 'fa fa-fw fa-fw-jet',
//            'class' => 'btn btn-xs btn-default ajax-get',
            'href' => url('article/index', ['search_field' => 'id', 'keyword' => '__aid__'])
        ];
...
//            ->addColumn('change_date', '修改时间')
    ...
    public function edit($id = null)
    {
        ...
        // 获取数据
        $info = ArticleContentModel::where('id', $id)
            ->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === ArticleContentModel::where('id', 'in', $ids)
                        ->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = ArticleContentModel::where("id", $id)
